...
const AddCourseForm = () => {
		...
		try {
			...
			if (response.ok) {
				Swal.fire({
					...
					text: `Felicidades, el curso ha sido creado exitosamente. ¡Comienza a explorar y gestionar tu nuevo curso ahora mismo!`,
	...
	return (
		<form onSubmit={handleSubmit} className="container mt-0 module-box box pb-40">
			...
			<div className="mb-3">
				<label className="form-label text-large">Duracion</label>
				<input
					type="number"
					name="duration"
			...
			<div className="mb-3">
				{categories && (
					<>
						<label>Seleccione las categorías correspondientes:</label>
						<select
